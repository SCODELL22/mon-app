...
import { OBJECTIF_CA_AGENCE, OBJECTIFS_COMMERCIAUX } from '@/lib/config';
import { euros, pct, dateFr } from '@/lib/format';
import { StatCard, BarTable, QuarterForecastChart } from '@/components/dashboard-ui';
import { PerformanceSection } from '@/components/performance-ui';

export const dynamic = 'force-dynamic';

/** « 2026-05 » -> « mai 2026 ». */
function periodeLabel(periode: string | null): string {
  if (!periode) return 'à date';
...
export default async function DashboardPage({
  ...
  searchParams: Promise<{ imported?: string; ignored?: string; factImported?: string }>;
}) {
  const { imported, ignored, factImported } = await searchParams;
  ...
  // Performance : CA & marge facturés vs objectif annuel, par commercial puis par client.
  const facturation = await listFacturation();
  const perf = buildPerformance(facturation, OBJECTIFS_COMMERCIAUX);
  const factMetas = await listFacturationMetas();
  const lastFact = factMetas[factMetas.length - 1];
  const factPeriode = periodeLabel(lastFact?.periode ?? '2026-05');
  ...
  return (
    <div className="space-y-8">
      ...
      {factImported && (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
          Facturation importée : {factImported} clients chargés.
        </div>
      )}
      ...
      <PerformanceSection data={perf} periodeLabel={factPeriode} snapshots={factMetas} />
