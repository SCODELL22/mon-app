...
export async function VueEntretien({
  espace,
  params,
  searchParams,
}: {
  ...
  const brut = await getOneOnOne(espace, id);
  if (!brut) notFound();
  ...
  if (!peutLireEntretien(a, brut)) return <AccesRefuse espace={espace} />;
  ...
  const [commercial, actions, tousSes1a1] = await Promise.all([
    getCommercial(espace, entretien.commercialId),
    listActions(espace, { oneOnOneId: entretien.id }),
    listOneOnOnes(espace, entretien.commercialId),
  ]);
  ...
  const actionsPrecedentes = precedent
    ? filtrerActionsPourLecteur(await listActions(espace, { oneOnOneId: precedent.id }), [precedent], a)
    : [];
  ...
  return (
    <Shell espace={espace} titre="Compte rendu" estManager={a.estManager} estAdmin={a.estAdmin}>
      <header
        style={{
          marginBottom: 18,
          display: 'flex',
      ...
      >
        <div>
          ...
          <p style={S.sub}>
            {commercial ? (
              <a href={`${base}/commercial/${commercial.id}`} style={S.link}>
                {commercial.nom}
              </a>
            ) : (
        ...
        {gestion && (
          <a href={`${base}/nouveau?id=${entretien.id}`} style={S.btn}>
            Modifier
          </a>
        )}
      </header>
      ...
      {gestion && !france && (
        ...
        >
          ...
          <form action={`/api/one-on-one/partage${qs}`} method="POST">
                  ...
                  {gestion && (
                    <td style={{ ...S.td, textAlign: 'right' }}>
                      {isActionOuverte(act.statut) && (
                        <form action={`/api/one-on-one/action${qs}`} method="POST">
                          <input type="hidden" name="id" value={act.id} />
                          <input type="hidden" name="statut" value="FAITE" />
                          <input type="hidden" name="retour" value={`${base}/entretien/${entretien.id}`} />
