...
export async function VueSaisie({
  espace,
  searchParams,
}: {
  espace: Espace;
  ...
  if (!peutEcrire(a)) return <AccesRefuse espace={espace} />;
  ...
  const commerciaux = (await listCommerciaux(espace)).filter((c) => gere(a, c.id));
  const entretien = id ? await getOneOnOne(espace, id) : null;
  ...
  if (entretien && !gere(a, entretien.commercialId)) return <AccesRefuse espace={espace} />;
  if (commercialParam && !gere(a, commercialParam)) return <AccesRefuse espace={espace} />;

  const commercialId = entretien?.commercialId ?? commercialParam ?? commerciaux[0]?.id ?? '';
  const commercial = commercialId ? await getCommercial(espace, commercialId) : null;
  ...
  if (commerciaux.length === 0) {
    return (
      <Shell espace={espace} titre={`Nouveau ${v.entretien}`} estManager estAdmin={a.estAdmin}>
  ...
  const pipeline = commercial
    ? await pipelineDuCommercial(espace, commercial, today)
    : null;

  // Actions encore ouvertes issues des séances PRÉCÉDENTES : c'est le point de départ du 1:1.
  const toutesActions = commercialId ? await listActions(espace, { commercialId }) : [];
  ...
  return (
    ...
    >
      ...
      <form action={`/api/one-on-one/entretien${qs}`} method="POST">
            ...
            <Card titre="Transcription Google Meet" accent={C.gm}>
              ...
              {extractionPossible ? (
                <div style={{ marginTop: 14 }}>
                  <button type="submit" formAction={`/api/one-on-one/extraction${qs}`} style={S.btn}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
          ...
          <a href={commercialId ? `${base}/commercial/${commercialId}` : base} style={S.btnGhost}>
            Annuler
          </a>
        </div>
      </form>
