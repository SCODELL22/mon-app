...
export async function VueFiche({
  ...
  const commercial = await getCommercial(espace, id);
  ...
  if (!peutVoirCommercial(a, commercial.id)) return <AccesRefuse espace={espace} />;
  ...
  const [brutEntretiens, brutActions, pipeline] = await Promise.all([
    listOneOnOnes(espace, commercial.id),
    listActions(espace, { commercialId: commercial.id }),
    pipelineDuCommercial(espace, commercial, today),
  ]);
  ...
  return (
    <Shell espace={espace} titre={commercial.nom} estManager={a.estManager} estAdmin={a.estAdmin}>
      <header
        style={{
          marginBottom: 18,
          display: 'flex',
      ...
      >
        ...
        {gestion && (
          <a href={`${base}/nouveau?commercial=${commercial.id}`} style={S.btn}>
            Nouveau {v.entretien}
          </a>
        )}
      </header>
      ...
      {!pipeline.rattache && (
        <Message ton="info">
          ...
          {a.estAdmin ? (
            <a href={`${base}/commerciaux?edit=${commercial.id}`} style={S.link}>
              la fiche
            </a>
          ) : (
                      ...
                      <td style={{ ...S.td, textAlign: 'right' }}>
                        <form action={`/api/one-on-one/action${qs}`} method="POST">
                          ...
                          <input
                            type="hidden"
                            name="retour"
                            value={`${base}/commercial/${commercial.id}`}
                          />
