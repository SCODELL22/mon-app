...
export async function POST(req: Request) {
  ...
  if (!extractionDisponible()) {
    return redirectTo(`${base}/nouveau?id=${entretien.id}&erreur=extraction-indisponible`);
  }
  if (!entretien.transcription.trim()) {
    return redirectTo(`${base}/nouveau?id=${entretien.id}&erreur=pas-de-transcription`);
  }

  let resultat;
  try {
    resultat = await extraireTrame(espace, entretien.transcription);
  } catch (e) {
    ...
    return redirectTo(`${base}/nouveau?id=${entretien.id}&erreur=${code}`);
  }
  ...
  const existantes = await listActions(espace, { oneOnOneId: entretien.id });
  ...
  return redirectTo(`${base}/nouveau?id=${entretien.id}&extrait=${remplies}&actions=${ajoutees}`);
}
